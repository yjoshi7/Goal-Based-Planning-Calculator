...
function renderCharts(goals) {
  ...
  chartInstances.donut = new Chart(donutCtx, {
    ...
    data: {
      ...
      datasets: [{ data: goals.map(g => Math.round(g.requiredSIP)), backgroundColor: ['#2563EB','#14B8A6','#F59E0B','#DC2626','#8B5CF6','#EC4899','#10B981','#6366F1','#F97316'] }]
    },
  ...
  chartInstances.stackedBar = new Chart(barCtx, {
    ...
    data: {
      labels: goals.map(g => g.name),
      datasets: [
        { label: 'FV of Savings & Contributions', data: goals.map(g => Math.round(g.totalFV)), backgroundColor: '#14B8A6' },
        { label: 'Gap to Fund', data: goals.map(g => Math.max(0, Math.round(g.shortfall))), backgroundColor: '#DC2626' }
      ]
    },
